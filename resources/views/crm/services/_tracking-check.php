...
function serviceTodoRowsCheck()
{
	$.ajax({
		type: "POST",
		url: "<?php echo PATH_WEB; ?>service-tracking/work-rows-check",
		...
		success: function(serviceEventsArray) {
			...
			for (i = 0; i < serviceEventTableRows.length; i++) 
			{
				...
				if(jQuery.inArray(serviceEventID, serviceEventsArray) === -1)
				{
					if(!$(serviceEventTableRows[i]).hasClass("service-todo-table-row-done"))
					{
						...
						$(serviceEventTableRows[i]).children("td:first").html("<strong style='color: #000;'>LEZÁRVA</strong><br>" + $(serviceEventTableRows[i]).children("td:first").html());
