...
@section("style")
	...
	.newcar-selling-todo-container .table-responsive{
		overflow-x: visible;
	}
	
	.newcar-selling-todo-table-container{
		width: 100%;
		overflow-x: auto;
	}
	...
	.newcar-selling-todo-table-container .bg-red, .bg-green{
		color: #000;
	}
@stop
...
@section("bodyBottom")
	...
	<script>
		$(".newcar-selling-todo-container .data-table").wrap( "<div class='newcar-selling-todo-table-container'></div>" );
		// $(".newcar-selling-todo-container .data-table").tableHeadFixer({'left' : 5});
	</script>
	<?php } ?>
	@include("crm.new-car-sellings._todo-check")
@stop
...
@section("content")
	...
	else
	{
		...
		<div class="newcar-selling-todo-container newcar-selling-todo-container-<?php echo $GLOBALS["URL"]->routes[1]; ?> service-todo-container-<?php echo $GLOBALS["URL"]->routes[1]; ?>">
		...
		function newcarSellingStatusChange(eventID)
		{
			if(eventID != "" && eventID != "undefined")
			{
				if($(".newcar-selling-todo-status-" + eventID).val() != "") { $(".newcar-selling-todo-question-" + eventID).hide(); }
				else { $(".newcar-selling-todo-question-" + eventID).show(); }
			}
		}
		function sendAnswer(eventID)
		{
			if(eventID != "" && eventID != "undefined")
			{
				if($(".newcar-selling-todo-status-" + eventID).val() != "")
				{
					$.ajax({
						type: "POST",
						url: "<?php echo PATH_WEB.$GLOBALS["URL"]->routes[0]; ?>/work-status",
						headers: {"X-CSRF-TOKEN": $("[name='_token']").val()},
						data: "status=" + $(".newcar-selling-todo-status-" + eventID).val() + "&comment=" + $(".newcar-selling-todo-comment-" + eventID).val() + "&eventID=" + eventID,
						dataType: "html",
						success: function(msg) {
							if(msg == "ok")
							{
