...
{
	foreach($attachments AS $attachment) { $emailController->attachments[] = ["path" => $attachment, "name" => NULL]; }
}

#Sending datas
$emailController->subject = "[GABLINI-CRM] ".$subject." (".$dateOut.")";
$emailController->body = $emailController->setBody("crm/service_custom_stats");

if(count($addresses) > 0) { $emailController->addresses = $addresses; }
else
{
	$webAddresses = new \App\Http\Controllers\WebAddressController;
	$addressList = $webAddresses->getAddressesForSendingByURL("crm-szerviz-negyedeves-statisztika-munkatarsak");
	$emailController->addresses = $addressList["all"];
}

if(!$watch) { $emailController->send(); }
else { echo $emailController->watch(); }
?>