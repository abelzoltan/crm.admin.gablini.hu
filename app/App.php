...
class App extends Cron
{
	...
	public function getUserByRegistrationConfirmHash($regConfirmHash, $delCheck = 1)
	{
		$row = $this->selectByField($this->tables("users"), "regConfirmHash", $regConfirmHash, "", $delCheck);		
		return ($row AND is_object($row) AND isset($row->id) AND !empty($row->id)) ? $row : false;
	}
	...
	public function getLastEventByEmail($email, $deleted = 0)
	{
		$query = "SELECT * FROM ".$this->tables("events")." WHERE email = :email";
		$params = [
			"email" => $email,
		];
